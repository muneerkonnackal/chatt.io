...
export const SocketContextProvider = ({ children }) => {
    ...
    const handleVoiceCall = useCallback(async (user) => {
        ...
        socket.emit('call', { participants, callType: 'voice' });
    }, [socket, currentSocketUser, ongoingCall, isAdmin]);
    ...
    const createPeer = useCallback((stream, initiator) => {
        ...
        // Handle incoming streams by adding them to remoteStreams array
        peer.on("stream", (remoteStream) => {
    ...
    const toggleScreenShare = useCallback(async () => {
        ...
        if (isScreenSharing) {
            ...
            try {
                ...
                // Add the screen share stream to the peer connection
                sp.addStream(newScreenStream);
                setScreenStream(newScreenStream);
                setIsScreenSharing(true);

                // Handle when the user stops screen sharing via the browser prompt
                newScreenStream.getVideoTracks()[0].onended = () => {
                    sp.removeStream(newScreenStream);
                    setScreenStream(null);
                    setIsScreenSharing(false);
                };
            } catch (error) {
    ...
    }, [peer, isScreenSharing, screenStream]);
    ...
    console.log('remote stream in context', remoteStreams);
