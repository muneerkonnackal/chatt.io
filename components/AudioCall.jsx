      ...
      {!isCallEnded && (
        <>
          <div className="mt-4 relative max-w-[800px] mx-auto">
            ...
            {localStream && (
              <VoiceContainer stream={localStream} isLocalStream={true} />
            )}
            {peer && peer.stream && (
              <VoiceContainer stream={peer.stream} isLocalStream={false} />
            )}
          </div>
          ...
          <div className="flex flex-col items-center mt-4">
            ...
            {user?.id === ongoingCall?.participants?.caller?.userId ? (
              <>
                <Avatar
                  src={ongoingCall.participants?.receiver?.profile?.imageUrl}
                />
                <h3>
                  {
            ...
            <motion.div
              initial={{ opacity: 0, y: -5 }} // Start hidden and slightly above
              animate={{ opacity: 1, y: 0 }} // Fade in and move into place
              transition={{ duration: 0.8, delay: 0.5 }} // Smooth transition with a delay
              className="mt-2 mb-2 font-semibold"
            >
              {peer && peer.stream && `${formatTime(elapsedTime)}`}
            </motion.div>
